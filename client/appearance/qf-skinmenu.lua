-- qf-skinmenu is an illenium-appearance replacement: it ships the same export
-- surface (getPedAppearance / setPlayerAppearance / setPedComponents /
-- setPedProps) but under its own resource name, so it needs its own adapter.
if (Config.Appearance == 'auto' and not checkResource('qf-skinmenu')) or (Config.Appearance ~= 'auto' and Config.Appearance ~= 'qf-skinmenu') then
    return
end
...
if Config.Debug then
    lib.print.info('[Appearance] Loaded: qf-skinmenu')
end

Bridge.Appearance = {}

Bridge.Appearance.fetchCurrentSkin = function()
    return exports['qf-skinmenu']:getPedAppearance(cache.ped)
end
...
Bridge.Appearance.setPlayerSkin = function(skinData)
    ...
    exports['qf-skinmenu']:setPlayerAppearance(skinData)
...
Bridge.Appearance.setPlayerClothing = function(clothingData)
    ...
    exports['qf-skinmenu']:setPedComponents(cache.ped, clothingData.components)
    exports['qf-skinmenu']:setPedProps(cache.ped, clothingData.props)
