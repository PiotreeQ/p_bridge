if (Config.Appearance == 'auto' and not checkResource('op-clothing')) or (Config.Appearance ~= 'auto' and Config.Appearance ~= 'op-clothing') then
    return
end
...
if Config.Debug then
    lib.print.info('[Appearance] Loaded: op-clothing')
end

Bridge.Appearance = {}

Bridge.Appearance.fetchCurrentSkin = function()
    return exports['op-clothing']:GetAppearance()
end

-- op-clothing keeps the client appearance synced with its database copy, and the
-- saved appearance is not exposed through a client-reachable callback.
Bridge.Appearance.fetchDatabaseSkin = function()
    local databaseSkin = exports['op-clothing']:GetAppearance()
...
Bridge.Appearance.convertSkinFormat = function(skinData)
    ...
    return {
        components = {
            ...
            {component_id = 3, drawable = skinData.arms or 0, texture = skinData.arms_2 or 0},            -- Arms
            ...
            {component_id = 7, drawable = skinData.chain_1 or 0, texture = skinData.chain_2 or 0},        -- Accessories
            {component_id = 8, drawable = skinData.tshirt_1 or 0, texture = skinData.tshirt_2 or 0},      -- Undershirt
            {component_id = 9, drawable = skinData.bproof_1 or 0, texture = skinData.bproof_2 or 0},      -- Body Armor
...
Bridge.Appearance.setPlayerSkin = function(skinData)
    ...
    exports['op-clothing']:SetAppearance(skinData)
...
Bridge.Appearance.setPlayerClothing = function(clothingData)
    ...
    exports['op-clothing']:SetPlayerClothing(clothingData)
